...
- FastAPI `/health` 和 `/ask` 接口已准备完成，可供 Chrome 扩展调用。
- Chrome Manifest V3 扩展已准备完成，默认连接本地 API。
...
已完成一个可运行的建筑知识库助手 MVP，支持本地文档上传、文档向量化检索、基于来源的回答生成，以及本地/云端模型切换；同时准备了 FastAPI 接口、Chrome 扩展、部署配置和下一轮评测方案。除非已经实际完成公开部署，不将其表述为“已获得公开演示链接”。
